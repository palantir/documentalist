 ...
 */
const { default: artifacts } = await import("./artifacts.json", {
  assert: { type: "json" },
});
...
if (process.env.GITHUB_API_TOKEN) {
  ...
  const currentGitCommitMessage = execSync(
  ...
  if (process.env.CIRCLE_PULL_REQUEST) {
    ...
    await octokit.rest.issues.createComment({
      ...
      issue_number: parseInt(
        basename(process.env.CIRCLE_PULL_REQUEST ?? ""),
        10
      ),
      body: commentBody,
    });
  ...
  Object.keys(ARTIFACTS).forEach((pkg) =>
    console.info(`${ARTIFACTS[pkg]}: ${getArtifactAnchorLink(pkg)}`)
  );
}
